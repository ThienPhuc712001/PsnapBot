#!/usr/bin/env python3
"""
Simple test script for PSnapBOT API connection - Windows compatible
"""
import sys
import os
...
try:
    ...
    print("[OK] Successfully imported config_agentrouter")
    
    print("\n=== AgentRouter Configuration Status ===")
    print_config_status()
    
    config = get_agentrouter_config()
    print(f"\n[OK] API Key configured: {'Yes' if config['api_key'] else 'No'}")
    ...
    if response.status_code == 200:
        ...
        if 'choices' in result and len(result['choices']) > 0:
            ...
            print("\n[SUCCESS] PSnapBOT API connection test PASSED!")
        else:
